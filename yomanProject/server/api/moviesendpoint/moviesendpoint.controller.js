...
export function updateMov(req, res) {
  if (req.body.Title) {
    delete req.body.Title;
  }
  return Moviesendpoint.findOneAndUpdate({Title: req.params.Title}, {$set:{Status:'True'}}).exec()
    .then(handleEntityNotFound(res))
    .then(saveUpdates(req.body))
    .then(respondWithResult(res))
    .catch(handleError(res));
...
export function updateMapping(req, res) {
  if (req.body.Title) {
    delete req.body.Title;
  }
  return Moviesendpoint.findOneAndUpdate({Title: req.params.Title}, {$set:{Status:'false'}}).exec()
    .then(handleEntityNotFound(res))
    .then(saveUpdates(req.body))
    .then(respondWithResult(res))
    .catch(handleError(res));
